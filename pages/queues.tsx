...
import QueueVisualization from '../components/animations/QueueVisualization'
import { Plus, Minus, RotateCcw } from 'lucide-react'

export default function Queues() {
  const [queue, setQueue] = useState([1, 5, 3])
  ...
  const animateEnqueue = async (value: number) => {
    setIsAnimating(true)
    const newSteps: string[] = []
    
    newSteps.push(`Step 1: Starting ENQUEUE operation for value ${value}`)
    setSteps([...newSteps])
    await sleep(1000)
    
    newSteps.push(`Step 2: Queue follows FIFO (First In, First Out)`)
    setSteps([...newSteps])
    await sleep(1000)
    
    newSteps.push(`Step 3: Current queue size: ${queue.length}`)
    setSteps([...newSteps])
    await sleep(1000)
    
    setNewElement(value)
    setOperation('enqueue')
    newSteps.push(`Step 4: Adding ${value} to the rear of queue`)
    setSteps([...newSteps])
    await sleep(1000)
    
    const newQueue = [...queue, value]
    setQueue(newQueue)
    newSteps.push(`Step 5: ✅ Successfully enqueued ${value}. Queue size is now ${newQueue.length}`)
    setSteps([...newSteps])
    await sleep(1000)
  ...
  const animateDequeue = async () => {
    ...
    newSteps.push(`Step 1: Starting DEQUEUE operation`)
    setSteps([...newSteps])
    await sleep(1000)
    
    newSteps.push(`Step 2: Current queue size: ${queue.length}`)
    setSteps([...newSteps])
    await sleep(1000)
    
    newSteps.push(`Step 3: Front element is ${frontElement}`)
    setSteps([...newSteps])
    setOperation('dequeue')
    await sleep(1000)
    
    newSteps.push(`Step 4: Removing ${frontElement} from front`)
    setSteps([...newSteps])
    await sleep(1000)
    
    const newQueue = queue.slice(1)
    setQueue(newQueue)
    newSteps.push(`Step 5: ✅ Successfully dequeued ${frontElement}. Queue size is now ${newQueue.length}`)
    setSteps([...newSteps])
    await sleep(1000)
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-2 h-[calc(100vh-80px)] flex flex-col">
          ...
          <div className="flex flex-col lg:flex-row gap-2 items-start lg:items-center">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1.5 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-600 disabled:cursor-not-allowed px-3 py-1.5 rounded-lg transition-colors text-sm whitespace-nowrap"
              >
                <Plus size={14} />
                Enqueue
              </button>
              <button
                ...
                disabled={queue.length === 0 || operation !== null}
              ...
              <div className="glass-card p-2 sm:p-3 bg-white/5">
                <div className="text-xs sm:text-sm space-y-2">
                  <div className="flex justify-between">
                    <span>Size:</span>
                    <span className="text-orange-400">{queue.length}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Front:</span>
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar">
                  {steps.length === 0 ? (
                    <div className="glass-card p-3 bg-white/5 text-center text-gray-400 text-xs">
                      Perform ENQUEUE or DEQUEUE to see execution steps
                    </div>
                  ) : (
                    steps.map((step, index) => (
                      <motion.div
                        ...
                        className="glass-card p-2 bg-white/5 border-l-4 border-orange-400"
                      >
        ...
        >
          <summary className="cursor-pointer p-3 sm:p-4 font-semibold text-base sm:text-lg hover:bg-white/5 rounded-lg transition-colors">
            Queue Properties (Click to expand)
          </summary>
          <div className="p-3 sm:p-4 pt-0">
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
            <div>
              <h3 className="text-base sm:text-lg font-semibold mb-2 text-orange-400">Operations</h3>
